// 云函数：setupDatabase - 创建所有新集合 + 写入预设标签
const cloud = require('wx-server-sdk')
cloud.init({ env: cloud.DYNAMIC_CURRENT_ENV })

const PRESET_TAGS = ['编程','设计','数据','AI','商业','金融','心理','沟通','写作','外语','职场','思维','技术','管理','营销','产品','创业']

const NEW_COLLECTIONS = ['tags', 'course_tags', 'user_tags', 'courses', 'lessons', 'messages', 'history']
...
exports.main = async () => {
  ...
  // 用 createCollection 逐个创建（与 setupCollections 相同的方式）
  for (const name of NEW_COLLECTIONS) {
    try {
      await db.createCollection(name)
      results.push({ collection: name, status: 'created' })
    } catch (e) {
      if (e.errCode === -501001 || String(e.message).includes('already exist') || String(e.message).includes('ALREADY_EXIST')) {
  ...
  // user_achievements 单独处理
  try {
    await db.createCollection('user_achievements')
    results.push({ collection: 'user_achievements', status: 'created' })
  } catch (e) {
    results.push({ collection: 'user_achievements', status: 'already_exists' })
  }
