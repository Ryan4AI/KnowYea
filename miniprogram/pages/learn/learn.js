...
Page({
  ...
  sendMessage(contentOverride, isAutoMessage) {
    ...
    const sysContent = [
      ...
      '',
      '# 课时完成标记（重要）',
      '课时内容讲解充分、用户已经理解后（用户答对了题、或表达了理解、或已有多轮有意义的对话），即可标记完成：',
      '在回复末尾加 {"action":"complete","score":N,"summary":"..."}',
      '',
      '# 评分标准',
      ...
      '# ⚠️ 重要限制（必须遵守）',
      '你只能讲解当前一课的内容。',
      '完成课时时只标记 JSON，不要在文字中问用户"是否继续下一课时"或"是否学习下一节"等引导问题。',
      '如果用户说"继续""下一课""下一个"等，请回应：',
      '"当前课时已完成，点击下方「进入下一节」按钮继续学习。"',
      '不要自行开始讲解下一节课的内容，不要创建新的课时编号。',
    ].join('\n')
